...
//
// 为什么只导出索引、不导出会话正文：索引是登记表，正文在 projects/<id>/sessions/ 下，
// 大小从几 KB 到几百 MB 不等、由用户自己决定要不要整体搬。合在一起会把一个
// 恒为几 KB 的备份变成一个体积不确定的包裹，也会把"备份"和"搬数据"两件事搅在一起。
// 分开之后语义很清楚：索引单独带走，工程列表立刻回来；正文没跟过来时由导入结果说实话。
